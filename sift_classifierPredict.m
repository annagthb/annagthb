%sift_classifierPredict
function [prediction conf]=sift_classifierPredict(cluster,img,pt)
    ...
    X=sift_extract(H,pt,xyCoords(:,1)',xyCoords(:,2)');
    % X=im2col(I,f.patchSize,'sliding');%extract the patch for each pixel as a column and store it in X
    %g to kanoniko extract to X prepei na ginei X'. g to sift prepei na
    %meinei X.
    ...
    %R=reshape(R,size(I)-f.patchSize+1);%reshape the resulted 6400 vector probabilities into a 80x80 matrix
...
%     ok1=classifier(1).prob;
%     ok2=classifier(2).prob;
end